...
class Deriver_Usecase3_Test extends Deriver_SystemTest_Template {
    ...
    void test1() throws IOException {
        canonicalPath = TestUtil.getTestDataPath() + "deriver" + File.separator + "usecase3";

        init();
        addCommonObjects();

        testobjectList.add(new TestPolicySpecification(abs, policy1, new TestRecord(false), new TestRecord(true)));
        testobjectList.add(new TestPolicySpecification(abs, policy2, new TestRecord(false), new TestRecord(true)));
